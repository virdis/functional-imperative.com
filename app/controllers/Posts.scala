...
object Posts extends Controller with MetaConfig {
  ...
  def byId(id: Int) = Action { request =>
    val r = Kleisli[SingleResult, PostService, Post] { postService =>
      val res = for {
        p <- postService.byId(id) |> SingleResult.createFromOption(s"Cannot find post for given id : ${id}")
      } yield p
      res
    }
    r.run(postService).run.map(_.fold(error => Ok(views.html.index(error)),
  ...
  def allActive = Action { request =>
    ...
    r.run(postService).fold(e => Ok(views.html.index(e)), posts => Ok(views.html.posts.posts(posts)))
  }
}